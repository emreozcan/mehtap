...
def test_time_now():
    with freeze_time(datetime.fromtimestamp(1600000000, timezone.utc), tz_offset=0):
        lua_time, = os_time()
...
def test_time_offset():
    with freeze_time(datetime.fromtimestamp(1600000000, timezone.utc), tz_offset=1):
        lua_time, = os_time()
        real_time = datetime.now().timestamp()
    assert lua_time.value == real_time == 1600000000
